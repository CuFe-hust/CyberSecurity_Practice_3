...
- **AI 默认只负责记录，不主动解题。** 题目思路、实验操作、Flag 由人类完成，AI 的主要任务是把用户口述/粘贴的过程整理成规范的 writeup 文档。
- **例外：当人类明确提出帮忙解题时才可解题。** 若用户明确说出“帮我解”“你来做”等请求，AI 可以参与解题；否则保持记录员角色。
- 用户会逐步提供操作步骤与截图（图片来自剪贴板临时目录），AI 负责核对截图内容与叙述的一致性，必要时指出矛盾之处（以实测/截图为准）。
- AI 可以解释漏洞原理、补充总结与修复建议，但默认不作为解题主导方。
